...
class SplitPane extends Component {
  constructor(props) {
    ...
    this.state = {
      sizes: this.props.sizes || this.getPanePropSize(props),
    };
  }

  componentWillReceiveProps(nextProps) {
    this.setState({sizes: nextProps.sizes||this.getPanePropSize(nextProps)});
  }
  componentDidMount(){
  ...
  componentWillUnmount() {
    ...
    // this.splitPane.removeEventListener('mouseleave', this.onMouseUp);
  }
  ...
  render() {
    ...
    const elements = children.reduce((acc, child, idx) => {
      ...
      if (isPane) {
        ...
        pane = <Pane animate={this.props.animate} {...paneProps}>{child}</Pane>;
      }
